...
static int cmpfunc2(data_t *d, int a, int b)
{
...
int main(int argc, char **argv)
{
    ...
    for (i = 8; i <= maxnum; i *= 2) {
        ...
        /* Sort first with different cmpfunc */
        t1 = gettime();
        sort(data, 
             0,                 /* Sort from this index */
             i-1,               /* up to and including this index */
             (cmpfunc_t) cmpfunc2, /* Cast to avoid compiler warning */
             (cpyfunc_t) cpyfunc, /* Cast to avoid compiler warning */
             sizeof(data_t));
        
        /* and then measure cost of sorting a reversed array */
        t1 = gettime();
        sort(data, 
             0,                 /* Sort from this index */
             i-1,               /* up to and including this index */
